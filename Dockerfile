...
RUN pip install --no-cache numpy scipy biopython parmed\
...
RUN cd /tmp \
    && wget -nv --progress=dot:giga http://ambermd.org/downloads/install_ambertools.sh \
...
RUN cd /tmp \
    && wget -nv --progress=dot:giga http://hex.loria.fr/dist800/hex-8.1.1-x64-mint17.2.run \
    && chmod +x hex-8*.run \
